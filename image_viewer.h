...
class ImageViewerApp {
  private:
	...
	sf::Vector2f render_origin; // POSITION WHERE QUAD WILL BE DRAWN AT
								// render_offsets {0, 0}
	...
	std::vector<sf::Vector2i>
		render_offsets; // offsets relative to render_origin
	...
	std::map<int, std::pair<std::atomic<unsigned int>, unsigned int>>
		n_pageside_available;
	...
	void prepare_texture(int image_index, float scale) {
		...
		loading_textures.try_emplace(key,
	...
	const sf::Texture &get_texture(int image_index, float scale) {
		...
		if (it == loaded_textures.end()) {
			...
			bool tex_available = loading_it != loading_textures.end() &&
								 loading_it->second.available();
			sf::Image tex_data;
			if (tex_available)
	...
	void update_status() {
		if (update_title || page_changed) {
			if (tags_indices.empty())
				...
				std::string base_filename = images[curr_image_index].substr(
					images[curr_image_index].find_last_of("/\\") + 1);
				std::string title = std::to_string(curr_tag) + " - " +
									base_filename + " [" +
									std::to_string(relative_index + 1) + "/" +
									std::to_string(tag_indices.size()) + "]";
	...
	void update_paging(int tag) {
		...
		if (curr_mode == ViewMode::Manga) {
			...
			for (int i = 0; i < (int)tag_indices.size(); i++) {
				...
				if (check_status > 0) {
					if (check_status == 2) {
						...
						// DON'T TRUST WIDE PAGES AS FIRST PAGE, USUALLY IT IS
						// NOT PART OF MANGA
						if (i > 1)
							start0++;
					}

					if (std::find(tag_repage_indices.begin(),
								  tag_repage_indices.end(),
								  tag_indices[i]) != tag_repage_indices.end())
						change_paging = !change_paging;

					if (texture_pageside[tag_indices[i]].available()) {
						auto [is_right, is_left] =
							texture_pageside[tag_indices[i]].get();
					...
					if (i + 1 == (int)tag_indices.size() || lone_page[i + 1]) {
						...
						if (change_paging)
							lone_page[streak_begin] =
								1 - lone_page[streak_begin];
			...
			tag_pages.clear();
			for (int i = 0; i < (int)tag_indices.size(); i++) {
				if (i + 1 == (int)tag_indices.size() || lone_page[i] ||
					lone_page[i + 1]) {
					tag_pages.emplace_back(std::vector{tag_indices[i]});
				} else {
					tag_pages.emplace_back(
						std::vector{tag_indices[i], tag_indices[i + 1]});
					i++;
				}
			}
		} else {
		...
		if (tag == curr_tag) {
			for (int i = 0; i < (int)tag_pages.size(); ++i) {
				if (std::find(tag_pages[i].begin(), tag_pages[i].end(),
							  curr_image_index) != tag_pages[i].end()) {
					curr_page_index = i;
					break;
				}
			}
	...
	void reset_scales_offsets() {
		...
		if (curr_mode == ViewMode::Manga || curr_mode == ViewMode::Single) {
			...
			sf::Vector2f drawn_size(0, textures_sizes[page[0]].y *
										   render_scales[0]);
			for (unsigned int i = 0; i < page.size(); i++)
				drawn_size.x += textures_sizes[page[i]].x * render_scales[i];

			sf::Vector2i offset =
				sf::Vector2i((window_size - drawn_size) / 2.f);
			...
			while (!lastpage) {
				...
				int offset_x =
					(window_size.x - textures_sizes[draw_index].x * scale) /
					2.f;
				...
				std::tie(draw_tag, draw_page_index, lastpage) =
					advance_page(draw_tag, draw_page_index, 1);
			}
		}
	}
	...
	void render() {
		...
		for (unsigned int i = 0; i < render_indices.size(); ++i) {
			sf::Sprite sprite(
				get_texture(render_indices[i], render_scales[i] * zoom_factor));
			sprite.setPosition(render_origin +
							   sf::Vector2f(render_offsets[i]) * zoom_factor);
			window.draw(sprite);

			used_textures.emplace_back(render_indices[i],
									   render_scales[i] * zoom_factor);
		}

		auto preload_page_offset = [this, &used_textures](int offset) {
			auto [preload_tag, preload_page_index, hit_border] =
				advance_page(curr_tag, curr_page_index, offset);
			if (hit_border)
				return;
		...
		std::erase_if(loaded_textures, [&used_textures](const auto &item) {
			return std::find(used_textures.begin(), used_textures.end(),
							 item.first) == used_textures.end();
		});
	...
	std::tuple<int, int, bool> advance_page(int tag, int page_index,
											int offset) {
		...
		while (offset != 0) {
			...
			int new_page_index =
				std::clamp(page_index + offset, 0, (int)tag_pages.size() - 1);
			...
			if (offset == 0)
				...
				int tag_pages_index =
					std::distance(pages.begin(), tag_pages_it);
				int new_pages_index = std::clamp(tag_pages_index + page_offset,
												 0, (int)pages.size() - 1);
	...
	void vertical_scroll(float offset) {
		...
		float scale = get_horizontal_fit_scale(image_index) * zoom_factor;
		...
		while (true) {
			if (-render_origin.y >= curr_image_height) {
				...
				std::tie(curr_tag, curr_page_index, hit_border) =
					advance_page(curr_tag, curr_page_index, 1);
				if (hit_border) {
					render_origin.y = -curr_image_height;
					break;
				}
			} else if (render_origin.y > 0) {
				std::tie(curr_tag, curr_page_index, hit_border) =
					advance_page(curr_tag, curr_page_index, -1);
				if (hit_border) {
					render_origin.y = 0;
		...
		if (old_tag != curr_tag || old_page_index != curr_page_index) {
			...
			while (true) {
				...
				int last_height = textures_sizes[last_index].y *
								  render_scales.back() * zoom_factor;
				int last_offset_y = render_offsets.back().y * zoom_factor;

				auto [last_tag, last_page_index, hit_border] = advance_page(
					curr_tag, curr_page_index, render_indices.size());
				if (render_origin.y + last_offset_y + last_height <
						window_size.y &&
					!hit_border) {
					int new_image_index = pages[last_tag][last_page_index][0];

					float scale = get_horizontal_fit_scale(new_image_index);
					int offset_x = (window_size.x -
									textures_sizes[new_image_index].x * scale) /
								   2.f;

					render_indices.push_back(new_image_index);
					render_offsets.emplace_back(
						offset_x, (last_offset_y + last_height) / zoom_factor);
					render_scales.push_back(scale);
	...
	void poll_events() {
		...
		while (window.pollEvent(event)) {
			if (event.type == sf::Event::Closed)
				...
				sf::View new_view(
					sf::FloatRect(sf::Vector2f(0.f, 0.f), window_size));
				window.setView(new_view);
			} else if (event.type == sf::Event::KeyPressed) {
				if (auto it = keyboard_bindings.find(
						{(int)event.key.code, curr_mode});
					it != keyboard_bindings.end())
					run_command(it->second);
				else if (auto it = keyboard_bindings.find(
							 {(int)event.key.code, ViewMode::Undefined});
						 it != keyboard_bindings.end())
					run_command(it->second);
				...
				if (auto it = mouse_bindings.find(
						{(int)event.mouseButton.button, curr_mode});
					it != mouse_bindings.end())
					run_command(it->second);
				else if (auto it =
							 mouse_bindings.find({(int)event.mouseButton.button,
												  ViewMode::Undefined});
						 it != mouse_bindings.end())
					run_command(it->second);
			}
		...
		for (auto it = n_pageside_available.begin(), next_it = it;
			 it != n_pageside_available.end(); it = next_it) {
			++next_it;
	...
	void handle_keyboard(float dt) {
		...
		if (keys_state[sf::Keyboard::Key::L])
			render_origin.x -= scroll_speed * dt;
	...
	void run_command(std::string_view cmd) {
		...
		if (action == "add_images") {
			...
			for (const auto &image_path : args) {
				...
				auto image_it =
					std::find(images.begin(), images.end(), image_path);
				...
				if (image_it == images.end()) {
					...
					texture_pageside.emplace_back(
						[&counter = it->second.first, image_path] {
				...
				tag_indices.insert(
					std::upper_bound(tag_indices.begin(), tag_indices.end(),
									 new_index,
			...
			if (tag_pages_it != pages.end()) {
				...
				int max_offset =
					std::distance(tag_pages_it, std::prev(pages.end()));
			...
			auto [prev_tag, prev_page_index] =
				std::tie(curr_tag, curr_page_index);
			auto [new_tag, new_page_index, hit_border] =
				advance_page(curr_tag, curr_page_index, offset);
			...
			if (!pages.empty() && curr_mode == ViewMode::Manga) {
				...
				if (auto it =
						std::find(curr_repage_indices.begin(),
								  curr_repage_indices.end(), curr_image_index);
					it != curr_repage_indices.end())
			...
			if (args[0] == "manga")
				...
				std::cerr << "Mode " << args[0] << " not recognized"
						  << std::endl;
  ...
  public:
	ImageViewerApp(std::string_view config_path) {
		...
		// sf::Style::Default);
		window.create(sf::VideoMode(sf::Vector2u(800, 600)), "image viewer",
					  sf::Style::Default);
